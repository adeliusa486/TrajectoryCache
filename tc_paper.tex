...
\begin{document}
	...
	\begin{table}[htbp]
		\centering
		\caption{\hl{Wall-Clock Execution Overhead and Real-Time RSU Profiling Across Caching Policies (16,346 Requests).}}
		\label{tab:computational_cost}
		\renewcommand{\arraystretch}{1.2}
		\resizebox{\linewidth}{!}{%
		\begin{tabular}{l c c l}
			\toprule
			\textbf{Policy Class} & \textbf{Time / Decision} & \textbf{Total Run Time} & \textbf{Real-Time Feasibility} \\
			\midrule
			LRU / FIFO ($\mathcal{O}(1)$ Recency) & $1.2 \;\mu\mathrm{s}$ & $1.51 \;\mathrm{s}$ & $\checkmark$ Ultra-light baseline \\
			LFU ($\mathcal{O}(1)$ Frequency) & $2.1 \;\mu\mathrm{s}$ & $1.76 \;\mathrm{s}$ & $\checkmark$ Minimal overhead \\
			\textbf{TrajectoryCache (Rule)} & $\mathbf{38.4 \;\mu\mathrm{s}}$ & $\mathbf{4.45 \;\mathrm{s}}$ & $\checkmark$ \textbf{Real-Time Compliant} \\
			QLearningCache (Online RL) & $42.8 \;\mu\mathrm{s}$ & $4.89 \;\mathrm{s}$ & $\checkmark$ Real-Time Compliant \\
			\bottomrule
		\end{tabular}%
		}
	\end{table}
