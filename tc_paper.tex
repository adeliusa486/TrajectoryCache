...
\begin{document}
	...
	\begin{frontmatter}
		...
		\title{Benchmarking Rule-Based and Learning-Based Cache Replacement in Vehicular Edge Computing}
	...
	\begin{figure*}[t]
		\centering
		\includegraphics[width=0.82\linewidth]{figures/system_architecture_fig1.png}
		\caption{\hl{System architecture of the comparative vehicular edge caching framework. Vehicular kinematic input $(p_v, v_v)$ subject to 0--15~m GNSS positioning noise and telemetry delay feeds into spatial urgency $U(f)$ and temporal popularity $P(f)$ extraction modules, which drive cache eviction decisions ($<43\,\mu\mathrm{s}$ execution time) under closed-form rule-based TrajectoryCache, online reinforcement learning QLearningCache, and classical baselines.}}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.98\linewidth]{figures/fig_miss_bar_fig2.png}
		\caption{Mean cache miss rate across 10 seeds at $\alpha=0.8$. TC ($W=0.2$) achieves the lowest miss rate (52.05\%). LFU is the strongest baseline (52.85\%); TC's 0.80~pp advantage over LFU is statistically significant ($p=0.042$, Wilcoxon signed-rank test, one-sided).}
		\label{fig:miss_bar}
	\end{figure}
	...
	\begin{table}[htbp]
		\centering
		\caption{\hl{Wall-Clock Execution Overhead and RSU Real-Time Profiling Across Eviction Policy Classes (16,346 Requests).}}
		\label{tab:computational_cost}
		\renewcommand{\arraystretch}{1.2}
		\small
		\begin{tabular}{l c c c l}
			\toprule
			\textbf{Policy Class / Algorithm} & \textbf{Time / Decision} & \textbf{Total Run Time} & \textbf{Overhead Factor} & \textbf{Real-Time Status} \\
			\midrule
			LRU / FIFO (Classical Recency) & $1.2 \;\mu\mathrm{s}$ & $1.51 \;\mathrm{s}$ & $1.00\times$ (Base) & $\checkmark$ Ultra-light ($\mathcal{O}(1)$) \\
			LFU (Frequency Tracking) & $2.1 \;\mu\mathrm{s}$ & $1.76 \;\mathrm{s}$ & $1.16\times$ & $\checkmark$ Minimal ($\mathcal{O}(1)$) \\
			\textbf{TrajectoryCache (Hybrid Rule)} & $\mathbf{38.4 \;\mu\mathrm{s}}$ & $\mathbf{4.45 \;\mathrm{s}}$ & $\mathbf{2.94\times}$ & $\checkmark$ \textbf{Real-Time Compliant} \\
			QLearningCache (Online RL AI) & $42.8 \;\mu\mathrm{s}$ & $4.89 \;\mathrm{s}$ & $3.24\times$ & $\checkmark$ Real-Time Compliant \\
			\bottomrule
		\end{tabular}
	\end{table}
